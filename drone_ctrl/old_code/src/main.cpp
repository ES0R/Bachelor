...
#define PIN_MOTOR_5            5 
#define PIN_MOTOR_6            2 

#define PIN_RC_RX_1            14 /* channel 1 (trust?) */ 
#define PIN_RC_RX_2            15 /* channel 2 ?? */ 
#define PIN_RC_RX_3            16 /* channel 3 ?? */ 
#define PIN_RC_RX_4            17 /* channel 4 ?? */ 
#define PIN_RC_RX_7            22 /* channel 7 ?? */ 
...
//   PORTB &= B11011111;
  ...
  digitalWriteFast(PIN_LED_DEBUG, true);

  Propeller[0].attach(PIN_MOTOR_1);
  Propeller[1].attach(PIN_MOTOR_2);
  Propeller[2].attach(PIN_MOTOR_3);
  Propeller[3].attach(PIN_MOTOR_4);
  Propeller[4].attach(PIN_MOTOR_5);
  Propeller[5].attach(PIN_MOTOR_6);
  ...
  while (true)
  { // to make sure the hardware it rdy 2 go
    if (imu.available())
      break;
    else
    {
      printf("# No IMU connection found\n");
      // usleep(100000);
    }
  }
}
