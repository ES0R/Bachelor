...
%% relates to drone2x2_sim.slx model
close all
clear
...
batteryMass = 0.3; % single battery
...
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/drone/trust_in',1,'openinput');
ios(2) = linio('drone2x2_sim/drone/height_control/heightVelLp',1,'openoutput');
setlinio(model,ios);
...
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/drone/height_control/height ctrl1',1,'openinput');
ios(2) = linio('drone2x2_sim/drone/height_control/h-filt-out',1,'openoutput');
setlinio(model,ios);
...
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/drone/roll_in',1,'openinput');
ios(2) = linio('drone2x2_sim/drone/roll-vel-deg',1,'openoutput');
setlinio(model,ios);
...
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/drone/roll ctrl',1,'openinput');
ios(2) = linio('drone2x2_sim/drone/roll-deg',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [2];
% Linearize the model
...
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/drone/u_pitch',1,'openinput');
ios(2) = linio('drone2x2_sim/drone/pitch-vel-deg',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [3];
% Linearize the model
...
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/drone/pitch ctrl',1,'openinput');
ios(2) = linio('drone2x2_sim/drone/pitch-deg',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [3];
% Linearize the model
...
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/drone/u_yaw',1,'openinput');
ios(2) = linio('drone2x2_sim/drone/yaw-vel-deg',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [3];
% Linearize the model
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% design yaw angle controller
UKp = 0; % remove undercarrage control
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/yaw ctrl',1,'openinput');
ios(2) = linio('drone2x2_sim/drone',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [2];
% Linearize the model
...
yaNi = 0;
yatiUse = 0;
[yaw, yakp, yatd] = findpd(Gyaw, yagm, yaal)
%[yaw, yakp, yati, yatd] = findpid(Gyaw, yagm, yaNi, yaal)
%% debug and save result
% showResult(plt, fil, fn, name, sys, w, kp, ti, Ni, td, al, gm, tle)
[C, Cd] = showResult(debugPlot,resultFile,filename, 'yaw angle', Gyaw, ...
           yaw, yakp, yati, yaNi, yatd, yaal, yagm, ...
           'Yaw angle controller');
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% design X velocity controller
UKp = 0; % remove undercarrage control
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/x-velocity',1,'openinput');
ios(2) = linio('drone2x2_sim/x-vel-measured',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [5];
% Linearize the model
...
for M = 1:size(op,2)
  [num,den] = ss2tf(sys.A(:,:,M), sys.B(:,:,M), sys.C(:,:,M), sys.D(:,:,M));
  Gxraw(M) = minreal(tf(num, den),0.05)
  bode(Gxraw(M),w);
  hold on
end
title('X-velocity (5sec)')
%legend('5', '6', '7', '8')
grid on


%% remove poles at higher frequency than XX rad/s
Gx1 = freqsep(Gxraw(1), 300)
bode(Gx1)
grid on
%% design ctrl
xvgm = 60;
...
xvtiUse = 0;
w = logspace(-1,1,1000);
[xvw, xvkp, xvtd] = findpd(Gx1, xvgm, xval,w)
%[xw, xkp, xti, xtd] = findpid(Gx, xgm, xNi, xal,w)
%% debug and save result
% showResult(plt, fil, fn, name, sys, w, kp, ti, Ni, td, al, gm, tle)
[C, Cd] = showResult(debugPlot,resultFile,filename, 'X-velocity', Gx1, ...
           xvw, xvkp, xvti, xvNi, xvtd, xval, xvgm, ...
           'X velocity controller');
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% design Y velocity controller
UKp = 0; % remove undercarrage control
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/y-velocity',1,'openinput');
ios(2) = linio('drone2x2_sim/y-vel-measured',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [4];
% Linearize the model
...
for M = 1:size(op,2)
  [num,den] = ss2tf(sys.A(:,:,M), sys.B(:,:,M), sys.C(:,:,M), sys.D(:,:,M));
  Gyvraw(M) = minreal(tf(num, den),0.05)
  bode(Gyvraw(M),w);
  hold on
end
title('Y-velocity')
%legend('3.1', '4', '5', '6', '7', '8', '9')
grid on
%% remove poles at higher frequency than XX rad/s
figure(101)
bode(Gyvraw(1),w)
hold on
Gyv = freqsep(Gyvraw(1), 300)
bode(Gyv,w)
legend('raw','reduced')
grid on
%% design ctrl
...
yvtd = 0;
w = logspace(-1,1,1000);
%[yvw, yvkp] = findp(Gyv, yvgm, w)
[yvw, yvkp, yvtd] = findpd(Gyv, yvgm, yval, w)
%[xw, xkp, xti, xtd] = findpid(Gx, xgm, xNi, xal,w)
%% debug and save result
% showResult(plt, fil, fn, name, sys, w, kp, ti, Ni, td, al, gm, tle)
[C, Cd] = showResult(debugPlot,resultFile,filename, 'Y-velocity', Gyv, ...
           yvw, yvkp, yvti, yvNi, yvtd, yval, yvgm, ...
           'Y velocity controller');
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% design X position controller
UKp = 0; % remove undercarrage control
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/x-position',1,'openinput');
ios(2) = linio('drone2x2_sim/x-measured',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [4];
% Linearize the model
...
for M = 1:size(op,2)
  [num,den] = ss2tf(sys.A(:,:,M), sys.B(:,:,M), sys.C(:,:,M), sys.D(:,:,M));
  Gxpraw(M) = minreal(tf(num, den),0.05)
  bode(Gxpraw(M),w);
  hold on
end
%legend('5', '6', '7','8','9')
grid on
%% remove poles at higher frequency than XX rad/s
Gxp = freqsep(Gxpraw(1), 300)
figure(103)
hold off
bode(Gxpraw(1),w)
hold on
bode(Gxp,w)
title('x-velocity - system, no ctrl')
legend('raw','limited')
grid on
%% design ctrl
xgm = 60;
...
xtiUse = 1;
w = logspace(-1,1,1000);
[xw, xkp, xtd] = findpd(Gxp, xgm, xal,w)
%[xw, xkp, xti, xtd] = findpid(Gxp, xgm, xNi, xal)
%% debug and save result
% showResult(plt, fil, fn, name, sys, w, kp, ti, Ni, td, al, gm, tle)
[C, Cd] = showResult(debugPlot,resultFile,filename, 'X-position', Gxp, ...
           xw, xkp, xti, xNi, xtd, xal, xgm, ...
           'X-position controller');

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% design Y position controller
UKp = 0; % remove undercarrage control
load_system(model);
open_system(model);
ios(1) = linio('drone2x2_sim/y-position',1,'openinput');
ios(2) = linio('drone2x2_sim/y-measured',1,'openoutput');
setlinio(model,ios);
% Use the snapshot times
op = [3];
% Linearize the model
...
for M = 1:size(op,2)
  [num,den] = ss2tf(sys.A(:,:,M), sys.B(:,:,M), sys.C(:,:,M), sys.D(:,:,M));
  Gypraw = minreal(tf(num, den),0.05)
  bode(Gxpraw,w);
  hold on
end
grid on
%% remove poles at higher frequency than XX rad/s
Gyp = freqsep(Gypraw, 300)
bode(Gyp,w)
legend('raw','limited')
grid on
%% design ctrl
...
w = logspace(-1,1,1000);
%[xw, xkp, xtd] = findpd(Gxp, xgm, xal,w)
[yw, ykp, yti, ytd] = findpid(Gyp, ygm, yNi, yal)
%% debug and save result
% showResult(plt, fil, fn, name, sys, w, kp, ti, Ni, td, al, gm, tle)
[C, Cd] = showResult(debugPlot,resultFile,filename, 'Y-position', Gyp, ...
...
figure(2000)
hold off
plot(mission.data(:,1), mission.data(:,2))
hold on
plot(mission.data(:,3), mission.data(:,4))
grid on