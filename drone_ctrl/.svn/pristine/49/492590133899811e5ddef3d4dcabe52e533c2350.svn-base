...
%%
trust2heightVel = tf(1,[totalMass 0]);
Ghv = Gtr * trust2heightVel
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
showResult(debugPlot,resultFile,filename, 'heightVel', Ghv, ...
           hvw, hvkp, hvti, hvNi, hvtd, hval, ...
           hvgm, 'motor to trust velocity');
%% implement height vel controller
% height velocity closed loop
Chvi = tf([hvti 1],[hvti 0]);
Chvd = tf([hvtd 1],[hvtd*hval 1]);
Chv = hvkp*Chvi;
Ghvcl =minreal(Chv*Ghv/(1+Chv*Ghv*Chvd));
% add integrator to actual height
Gh = Ghvcl * tf(1,[1 0]);
%% height controller
hgm = 90;
hal = 0.1;
...
Chd = tf([htd 1],[hal*htd 1]);
%Chi = tf([hti 1],[hti 0]);
Ch = hkp*Chd;
Chcl = minreal(hkp*Gh/(1+Ch*Gh));
%% debug and save result
% showResult(plt, fil, fn, name, sys, w, kp, ti, Ni, td, al, gm, tle)
showResult(debugPlot,resultFile,filename, 'heightPos', Gh, ...
           hw, hkp, hti, hNi, htd, hal, ...
           hgm, 'Height velocity (m/s) to height (m)');
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Model extension to roll and pitch input
% roll trust share
% moment motor 1
