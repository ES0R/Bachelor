...
{
public:
  /// markerId is the ID of the marker extracted from the parameter markerIds found på the cv::aruco::detectMarkers function
  int markerId = -1;
...
public:
  /** constructor 
   * \param iCam is pointer to source camera. 
   * \param database is parent object
   * \param srcID is ID for messages coming from this data object */
  ArUcoVals(UCamera * iCam, UDataBase * database, int srcID)
  : UData(database)
  {
    cam = iCam;
    sourceID = srcID;
...
protected:
  /**
   * on-line help with keywords */
  void sendHelp(const int client) override;
  /**
   * Update status string */
  void updateArUcoString();
